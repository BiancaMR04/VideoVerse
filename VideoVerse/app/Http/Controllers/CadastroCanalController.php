...
class CadastroCanalController extends Controller
{
    ...
    public function editarCanal(Request $request, $id){
        ...
        if((empty($nomeCanal) || $canal->nome == $nomeCanal) && (empty($descricao) || $canal->descricao == $descricao) && !$request->hasFile('foto_perfil') && !$request->hasFile('foto_fundo')){
            $msg = 'Você não alterou nenhum campo!';
            return view('criar_canal', ['msg' => $msg, 'canal' => $canal]);
        }
